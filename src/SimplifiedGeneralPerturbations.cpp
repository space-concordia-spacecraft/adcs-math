...
namespace adcs::sgp
{
            ...
            )
    {
        ...
        if (init == 'n')
        {
            ...
            else
            {
                ...
                alfdp = alfdp + dalf;
                betdp = betdp + dbet;
                nodep = fmod(nodep, CONST_TWO_PI);
                //  sgp4fix for afspc written intrinsic functions
                // nodep used without a trigonometric function ahead
                if ((nodep < 0.0) && (opsmode == 'a'))
                    nodep = nodep + CONST_TWO_PI;
    ...
    *    vallado, crawford, hujsak, kelso  2006
            ...
            )
    {
        ...
        xnodce = fmod(4.5236020 - 9.2422029e-4 * day, CONST_TWO_PI);
        stem = sin(xnodce);
        ctem = cos(xnodce);
        ...
        zmol = fmod(4.7199672 + 0.22997150  * day - gam, CONST_TWO_PI);
        zmos = fmod(6.2565837 + 0.017201977 * day, CONST_TWO_PI);
    ...
    *    vallado, crawford, hujsak, kelso  2006
            ...
            )
    {
        ...
        if ((inclm < 5.2359877e-2) || (inclm > CONST_PI - 5.2359877e-2))
            shs = 0.0;
        if (sinim != 0.0)
            shs = shs / sinim;
        sgs = sghs - cosim * shs;
        ...
        dndt = 0.0;
        theta = fmod(gsto + tc * rptim, CONST_TWO_PI);
        em = em + dedt * t;
        inclm = inclm + didt * t;
        argpm = argpm + domdt * t;
        nodem = nodem + dnodt * t;
        ...
        if (irez != 0)
        {
            ...
            if (irez == 2)
            {
                ...
                xlamo = fmod(mo + nodeo + nodeo - theta - theta, CONST_TWO_PI);
            ...
            if (irez == 1)
            {
                ...
                xlamo = fmod(mo + nodeo + argpo - theta, CONST_TWO_PI);
    ...
    *    vallado, crawford, hujsak, kelso  2006
            ...
            )
    {
        ...
        c1p2p = c1 + CONST_TWO_PI;
        double gsto1 = fmod(thgr70 + c1*ds70 + c1p2p*tfrac + ts70*ts70*fk5r, CONST_TWO_PI);
        if (gsto1 < 0.0)
            gsto1 = gsto1 + CONST_TWO_PI;
        //    }
        //    else
    ...
    *    vallado, crawford, hujsak, kelso  2006
            ...
            )
    {
        ...
        if ((omeosq >= 0.0) || (satrec.no_unkozai >= 0.0))
        {
            ...
            /* --------------- deep space initialization ------------- */
            if ((CONST_TWO_PI / satrec.no_unkozai) >= 225.0)
            {
                satrec.method = 'd';
                satrec.isimp = 1;
                tc = 0.0;
    ...
    *    vallado, crawford, hujsak, kelso  2006
            ...
            )
    {
        ...
        if (satrec.method == 'd')
        {
            ...
            {
                xincp = -xincp;
                nodep = nodep + CONST_PI;
                argpp = argpp - CONST_PI;
            }
            if ((ep < 0.0) || (ep > 1.0))
            {
                //            printf("# error ep %f\n", ep);
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    {
        const double xpdotp = 1440.0 / (CONST_TWO_PI);  // 229.1831180523293
        ...
        satrec.inclo = satrec.inclo  * CONST_DEG_TO_RAD;
        satrec.nodeo = satrec.nodeo  * CONST_DEG_TO_RAD;
        satrec.argpo = satrec.argpo  * CONST_DEG_TO_RAD;
        satrec.mo = satrec.mo     * CONST_DEG_TO_RAD;
    ...
    * --------------------------------------------------------------------------- */
            ...
            )
    {
        ...
        magv1 = glm::length(vec_1);
        magv2 = glm::length(vec_2);

        if (magv1*magv2 > small*small)
        {
            temp = glm::dot(vec_1, vec_2) / (magv1*magv2);
            if (fabs(temp) > 1.0)
    ...
    * --------------------------------------------------------------------------- */
            ...
            )
    {
        ...
        if (ecc > 1.0 + small)
        {
            if ((ecc > 1.0) && (fabs(nu) + 0.00001 < CONST_PI - acos(1.0 / ecc)))
            {
            ...
            // ----------------- parabolic ---------------------
        if (fabs(nu) < 168.0*CONST_PI / 180.0)
        {
            e0 = tan(nu*0.5);
            m = e0 + (e0*e0*e0) / 3.0;
        }
        ...
        if (ecc < 1.0)
        {
            m = fmod(m, 2.0 *CONST_PI);
            if (m < 0.0)
                m = m + 2.0 *CONST_PI;
            e0 = fmod(e0, 2.0 *CONST_PI);
        }
    }  // newtonnu
    ...
    * --------------------------------------------------------------------------- */
            ...
            )
    {
        ...
        magh = glm::length(hbar);
        if (magh > small)
        {
            nbar[0] = -hbar[1];
            nbar[1] = hbar[0];
            nbar[2] = 0.0;
            magn = glm::length(nbar);
            ...
            {
                // ----------------  circular equatorial ---------------
                if ((incl < small) | (fabs(incl - CONST_PI) < small))
                {
                    //#ifdef _MSC_VER
